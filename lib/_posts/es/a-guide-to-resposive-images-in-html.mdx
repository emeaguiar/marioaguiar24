...
updatedOn: '2025-10-04T21:52:19.061Z'
...
import { SrcsetExample, PictureExample } from '@/examples/a-guide-to-responsive-images-in-html/es';
...
- [FORMATOS DE IMÁGENES](#formatos-de-imágenes)
- [WEBP (FIGURAS E IMÁGENES REPRESENTATIVAS)](#webp-figuras-e-imágenes-representativas)
- [CARGA DIFERIDA (*Lazy Loading*)](#carga-diferente-lazy-loading)
- [IMÁGENES RESPONSIVAS](#imágenes-responsivas)
  - [SOURCE SET (`srcset`)](#source-set-srcset)
  - [LA RELACIÓN `srcset` Y `sizes`](#la-relación-srcset-y-sizes)
  - [EL ELEMENTO `<picture>`](#el-elemento-picture)
...
A diferencia de `srcset` y `sizes`, `<picture>` funciona más como un contenedor de nuestro elemento `<img>`. Cuando usamos `<picture>`, el navegador no *elige* la mejor imagen a utilizar, en lugar de eso, se limita a obedecer las indicaciones que nosotros le brindamos. Es por esto que no es tan genérico como el ejemplo anterior, pero si más sencillo de utilizar:

```html
<picture>
	<source srcset="https://picsum.photos/800/450" media="(min-width: 700px)" />
	<img src="https://picsum.photos/400/225" alt="Imagen Responsiva" />
</picture>
```

Esto nos daría un resultado como:

<AlignWide>
  <PictureExample />
</AlignWide>

En este caso, especificamos que si nuestra pantalla es de al menos 700px entonces debemos mostrar la imagen asignama al atributo `srcset` del elemento `<source>`. El navegador simplemente tomará esta indicación y mostrará la imagen, sin tener que tomar desciciones por su cuenta.

> [!NOTE]
> El atributo `media` soporta cualquier *media query*, ya sea de dispositivo, tamaño de *viewport*, o incluso de contenedor. por ejemplo:
> ```html
> <source srcset="https://picsum.photos/225/400" media="(orientation: portrait)" />
> ```

## CONCLUSIÓN
En un mundo donde nuestras páginas de internet pueden ser visualizadas en prácticamente cualquier tipo de dispositivo, y con tantas opciones a nuestro alcance, no olvidemos que también debemos procurar que nuestros usuarios 
