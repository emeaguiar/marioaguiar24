...
updatedOn: '2024-10-17T16:38:38.864Z'
description: >-
...
> **¿Qué es `debounce`?**
> 
> Cuando hablamos de debounce, nos referimos a una técnica utilizada en programación (en Javascript principalmente) en la que **evitamos que un evento se ejecute hasta que cierto tiempo ha pasado desde la primera vez que se ejecutó**. Esto nos ayuda a prevenir eventos duplicados y efectos inesperados en nuestra aplicación.
...
![Console log de cada cambio en el campo de búsqueda](/posts/implementing-debounce-in-javascript/no-debounce.gif)
...
> [!NOTE]
> La magia consiste en que en lugar de ejecutar nuestra función inmediatamente, nos esperamos a que **el temporizador termine**, y cada vez que ese mismo evento es repetido el timer vuelve a reiniciarse. Así nunca se repite un evento ya que el timer no lo permitirá.

Ya que tenemos la lógica que necesitamos, podemos pasar a la parte divertida.
## La implementación

Ya que este es un tutorial para **mostrar el funcionamiento de debounce**, no recurriré a mi método habitual para utilizarlo (ya que mi método habitual es simplemente usar [lodash](https://lodash.com/docs/4.17.15#debounce)), en lugar de eso, tomaremos la lógica que escribimos en los párrafos anteriores y la traduciremos a Javascript.
...
<Code
	language="js"
	children={`const debounce = ( callback, delay ) => {
...
> crear una referencia a `timer` que sólo sea accesible para nuestra función `debounce`, y no para el
> resto de nuestro código. De esta forma, **podemos mantener el estado de nuestro timer entre llamadas**.
> 
> Además, usamos `(...args)` para decir que nuestra función `debounce` puede recibir **cualquier número de argumentos**,
> y asi mismo los pasará a nuestra función `callback`.

Para utilizarla con nuestro ejemplo, en lugar de iniciar nuestra función original, lo que hacemos es **envolverla en nuestra función de `debounce`**, de esta forma:

<Code
	language="jsx"
	children={`const debounced = debounce((e) => {
    console.log(e.target.value);
}, 500);`}
/>
...
<Code
	language="js"
	children={`input.addEventListener('change', debounced)`}
/>
